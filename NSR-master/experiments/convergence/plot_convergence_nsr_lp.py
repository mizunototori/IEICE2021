...
def plot_cost(file_name, cost, k):
    fig, ax = plt.subplots()
    ax.set_title('Cost of NN-MU-lp: k=' + str(k))
...
def plot_diff_tol(file_name, diff_tol, k):
    fig, ax = plt.subplots()
    ax.set_title('Difference of tolerance of NN-MU-lp: k=' + str(k))
    ax.set_xlabel('Number of iterations')
...
def main():
    k = 11
    file_name = 'fig_convergence_nsr_lp_k_' + str(k)
    cost, i = files2cost('result_lp/', k)

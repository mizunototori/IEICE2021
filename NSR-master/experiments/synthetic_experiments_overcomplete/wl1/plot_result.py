...
def load_and_mean_est_logs(alpha, keyword):
    ...
    for n_iter in range (1, n_n_iter):
        print('n_iter:', n_iter)
        file_name = "results/p_09/" + str(constraint) + "_alpha_" + str(alpha) + "_p_" + str(p) + "_" + str(n_iter) + ".pickle"
...
n_alpha = 10
log_means = []
#legend_tuple =  tuple([str(round(a * 0.1, 1)) for a in range(4, 10)])
legend_tuple =  tuple([str(round(a * 0.1, 1)) for a in range(1, n_alpha)])

for a in range(1, n_alpha):
    alpha = round(a * 0.1, 1)
    log_means.append(load_and_mean_est_logs(alpha, keywords[2]))

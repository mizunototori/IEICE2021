...
def l1_exam(alpha, p, constraint, file_name):
    n_features = 20
    n_components = 50
    n_samples = 1500
    n_nonzero_coefs = 3
    ...
    model = NSR(n_components=n_components, solver='mu',
                constraint=constraint, alpha=alpha, p=p,
...
if __name__ == '__main__':
    ...
    for n_iter in range(0, 50):
        for p in ps: 
            for a in range(1, 10):
                alpha = round(a * 0.1, 1)
                file_name = "results/" + str(constraint) + "_alpha_" + str(alpha) + "_p_" + str(p) + "_" + str(n_iter) + ".pickle"
                l1_exam(alpha, p, constraint, file_name)

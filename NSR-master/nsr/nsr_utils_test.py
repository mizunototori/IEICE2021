...
def _hoyers_sparsity(A):
    ...
    def vector_sp(A):
        N = len(A)
        numerator_1_old = np.sqrt(N) - (sum(abs(A.T)))
        numerator_2_old = (np.sqrt(sum(A.T ** 2)) + EPSILON)
        numerator_old = numerator_1_old / (numerator_2_old + EPSILON)
        denominator_old = np.sqrt(N) - 1
        sp_old = min(1, max(0, abs(numerator_old / (denominator_old + EPSILON))))

        numerator_1 = np.sqrt(N) - np.sum(np.abs(A))
        numerator_2 = np.sqrt(np.sum(A ** 2)) + EPSILON
        numerator = numerator_1 / (numerator_2 + EPSILON)
        denominator = np.sqrt(N) - 1.0
        sp = min(1.0, max(0.0, np.abs(numerator / (denominator + EPSILON))))

        assert sp-sp_old<1e-07, 'Numerical error in Hoyar\'s sparsity'
